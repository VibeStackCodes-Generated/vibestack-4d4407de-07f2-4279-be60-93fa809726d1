...
import { t as require_jsx_runtime } from "./jsx-runtime-BgCp37or.js";
			...
			switch (segment[0]) {
				...
				case SEGMENT_TYPE_PARAM: {
					...
					if (existingNode) nextNode = existingNode;
						...
						const next = createDynamicNode(SEGMENT_TYPE_PARAM, route.fullPath ?? route.from, actuallyCaseSensitive, prefix, suffix);
						nextNode = next;
						next.depth = depth;
						next.parent = node;
						node.dynamic ??= [];
				...
				case SEGMENT_TYPE_OPTIONAL_PARAM: {
					...
					if (existingNode) nextNode = existingNode;
						...
						const next = createDynamicNode(SEGMENT_TYPE_OPTIONAL_PARAM, route.fullPath ?? route.from, actuallyCaseSensitive, prefix, suffix);
						nextNode = next;
						next.parent = node;
						next.depth = depth;
						node.optional ??= [];
				...
				case SEGMENT_TYPE_WILDCARD: {
					...
					const prefix = !prefix_raw ? void 0 : actuallyCaseSensitive ? prefix_raw : prefix_raw.toLowerCase();
					const suffix = !suffix_raw ? void 0 : actuallyCaseSensitive ? suffix_raw : suffix_raw.toLowerCase();
					const next = createDynamicNode(SEGMENT_TYPE_WILDCARD, route.fullPath ?? route.from, actuallyCaseSensitive, prefix, suffix);
					nextNode = next;
					next.parent = node;
					next.depth = depth;
					node.wildcard ??= [];
...
function resolvePath({ base, to, trailingSlash = "never", cache }) {
	...
	for (let i = 0; i < baseSegments.length; i++) {
		...
		if (kind === SEGMENT_TYPE_PARAM) joined += prefix || suffix ? `${prefix}{$${value}}${suffix}` : `$${value}`;
		else if (kind === SEGMENT_TYPE_WILDCARD) joined += prefix || suffix ? `${prefix}{$}${suffix}` : "$";
	...
	constructor(options) {
		...
		this.buildLocation = (opts) => {
			const build = (dest = {}) => {
				...
				if ((destMatchResult.foundRoute ? destMatchResult.foundRoute.path !== "/" && destMatchResult.routeParams["**"] : trimPathRight(interpolatedNextTo)) && this.options.notFoundRoute) destRoutes = [...destRoutes, this.options.notFoundRoute];
				...
				const nextPathname = opts.leaveParams ? nextTo : decodePath(!changedParams ? interpolatedNextTo : interpolatePath({
					path: nextTo,
					params: nextParams,
					decoder: this.pathParamsDecoder,
					server: this.isServer
		...
		this.resolveRedirect = (redirect2) => {
			...
			if (redirect2.options.href && !redirect2.options._builtLocation && isDangerousProtocol(redirect2.options.href, this.protocolAllowlist)) throw new Error(`Redirect blocked: unsafe protocol in href "${redirect2.options.href}". Allowed protocols: ${Array.from(this.protocolAllowlist).join(", ")}.`);
		...
		const externalLink2 = (() => {
			if (hrefOption2?.external) {
				if (isDangerousProtocol(hrefOption2.href, router.protocolAllowlist)) {
					console.warn(`Blocked Link with dangerous protocol: ${hrefOption2.href}`);
					return;
				}
...
var Match = import_react.memo(function MatchImpl({ matchId }) {
	...
	}), matchState.parentRouteId === rootRouteId && router.options.scrollRestoration ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(OnRendered, {}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ScrollRestoration$1, {})] }) : null] });
});
function OnRendered() {
	const router = useRouter();
...
var MatchInner = import_react.memo(function MatchInnerImpl({ matchId }) {
	...
	if (match.status === "error") {
		if (isServer ?? router.isServer) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)((route.options.errorComponent ?? router.options.defaultErrorComponent) || ErrorComponent, {
			error: match.error,
			reset: void 0,
			info: { componentStack: "" }
		});
...
function Matches() {
	...
	const pendingElement = PendingComponent ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(PendingComponent, {}) : null;
	const inner = /* @__PURE__ */ (0, import_jsx_runtime.jsxs)((isServer ?? router.isServer) || typeof document !== "undefined" && router.ssr ? SafeFragment : import_react.Suspense, {
		fallback: pendingElement,
		children: [!(isServer ?? router.isServer) && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Transitioner, {}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(MatchesInner, {})]
	});
